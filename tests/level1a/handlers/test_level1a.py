...
def test_get_or_raise_raises():
    with pytest.raises(
        EnvironmentError, match="DEFINITELYNOT is a required environment variable"
    ):
...
@pytest.mark.parametrize(
    "indices,first,last,expect",
    (
        (  # returns True when covers
            pd.DatetimeIndex([pd.Timestamp(2022, 12, 1), pd.Timestamp(2022, 12, 4)]),
...
@pytest.mark.parametrize(
    "eval_point,indices,values,expect",
    (
        (1, np.array([1, 2]), np.array([1, 2]), 1),
        (
            2,
            np.array([1, 3]),
        ...
        (3, np.array([3, 3]), np.array([10, 10]), 10),
    ),
)
...
def test_interpolate():
    ...
    pd.testing.assert_frame_equal(
        ...
        pd.DataFrame([np.nan, 1.5, 3.0, 3.75, np.nan], index=datetimes),
    )


def test_interpolate_with_max_diff_returns_nan():
    datetimes = pd.DatetimeIndex(
        ["2022-11-01T06:00:00", "2022-11-02T00:00:00", "2022-11-02T18:00:00"]
    )
    dataframe = pd.DataFrame(
        [1.0, 2.0, 3.0, 4.0],
        index=pd.DatetimeIndex(
    ...
    pd.testing.assert_frame_equal(
        ...
        pd.DataFrame([1.5, 3.0, np.nan], index=datetimes),
    )
...
def test_disambiguate_matches(csv_data: dict, expected: str):
    ...
    pd.testing.assert_frame_equal(
        matches, dataframe[dataframe["schedule_xml_file"] == expected]
    )
...
def test_find_match(schedule: pd.DataFrame):
    ...
    answer = find_match(target_date=target_date, column="Answer", dataframe=schedule)
    assert answer == 42
...
def test_find_match_warns_on_missing(schedule: pd.DataFrame):
    ...
    with pytest.warns(MissingSchedule, match="Missing schedule"):
        find_match(target_date=target_date, column="Answer", dataframe=schedule)
...
def test_dropna_arrays():
    dataframe = pd.DataFrame.from_dict(
        {"Squared": [[np.nan, 1764], [1764, 1764]], "Answer": [-42, 42]}
    )
    pd.testing.assert_frame_equal(
        dropna_arrays(dataframe, columns=["Squared"]), dataframe[dataframe.Answer == 42]
    )
                        ...
                        "Records": [
                            {
                                "s3": {
                                    "bucket": {
                                        "name": str(
                                            Path(__file__).parent / "files" / "rac"
                                        )
                                    },
